...
/// @title Auditing smart contract for the Melon Protocol.
contract Auditing is AuditingInterface {
    ...
    /// Returns true if the auditor is approved for this contract.
    ...
    returns (uint256 insertIndex) {
        ...
        if (auditedTimespans.length == 0 || auditedTimespans[auditedTimespans.length-1].start <= _audit.timespanStart) {
            ...
            // irregular audit, might be inserted in between existing timespans
    ...
    function removeAuditedTimespan(Timespan[] storage auditedTimespans, uint index) 
    private {
