...
const getAuditsFromFund = async (environment, {
  ...
  const auditingContract = await environment.api.newContract(abi, auditingContractAddress);

  const len = await auditingContract.instance.getLength.call({}, [fundAddress]);
  console.log(len, len.toNumber());

  //console.log(await auditingContract.instance.getByIndex.call({}, [fundAddress, 0]));
  const auditPromises = R.range(0, len.toNumber()).map(i =>
    auditingContract.instance.getByIndex.call({}, [fundAddress, i]).then(([auditor, dataHash, timespanStart, timespanEnd, opinion]) => ({
      auditor,
      dataHash: dataHash.reduce((carry, current) => carry+current.toString(16), "0x"),
